let support_files = (_flags, out_dir) => {
  ...
  Logs.debug(m => m("Making sure output dir $out_dir exists..."));
...
let build = (_flags, out_dir, pkg_name, src_dir, lib_dir) => {
  ...
  Os.mkdirp(out_dir) |> ignore;
  let cmti_odocs =
    package.cmtis |> List.map(Odoc.cmti_to_odoc(lib_dir, pkg_name));
  let mld_odocs =
    List.map(Odoc.mld_to_odoc(lib_dir, src_dir, pkg_name), package.mlds);
  let odocs = cmti_odocs @ mld_odocs;
  let _htmls = List.map(Odoc.odoc_to_html(lib_dir, out_dir), odocs);
  ();
};
/*
 let build = (flags, out_dir, pkg_name, src_dir, lib_dir) => {
   ...
   Logs.info(m => m("Compiling documentation for package \"%s\"...",pkg_name));

   Logs.debug(m => m("Making sure output dir \"%s\" exists...", out_dir));
   Os.mkdirp(out_dir);

   Logs.debug(m => m("Looking for .cmti files in $lib_dir"));
   Logs.debug(m => m("Compiling .cmti files into .odoc files..."));
   let cmti_odocs =
     package.cmtis |>
     List.map(Odoc.cmti_to_odoc(lib_dir, pkg_name));
   Logs.debug(m => m("Done ✅")));

   Logs.debug(m => m("Looking for .mld files in $src_dir");
